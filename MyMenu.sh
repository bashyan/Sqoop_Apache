...
while [ opt != '' ]
    ...
    if [[ $opt = "" ]]; then 
        ...
        sqoop list-databases --connect "jdbc:mysql://localhost" --username root --password 4rt6y;
        show_menu;
        ;;

        2) clear;
        option_picked "Now lets import the AppData table in our hdfs file system ";
        sqoop import --connect jdbc:mysql://localhost/utility --username root --password '4rt6y' --table AppData --target-dir /usecase/AppData;
        show_menu;
        ;;
            
        3) clear;
        option_picked "Now We can import the utility DB in our Hive table Structure to be able to Query";
        sqoop import --connect "jdbc:mysql://localhost/utility" --username root --password '4rt6y' --table AppData --hive-import --hive-table utility.AppData ;
        show_menu;
        ;;
	    ...
	    case $n in
                1)	echo "Please Enter the bank name"
                    read bankName                    	
                    hive -e "Select * from utility.AppData where PinCode = $pinCode AND bank = '${bankName}'"
                    ;;			
